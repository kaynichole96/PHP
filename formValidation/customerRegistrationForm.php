<?php
	...
	if( isset($_POST['submitButton']) ) {
		...
		$formValidations = new FormValidator();
		...
		if( $formValidations->isEmpty($customerName) ) {
			$validForm = false;
			$customerNameErrorMessage = "Please enter your Name";
		}
		if( $formValidations->isNullOrUndefined($customerName) ) {
			$validForm = false;
		...
		if ( !$formValidations->strLengthCheck(10, 12, $customerPhone) ) {
			$validForm = false;
			$customerPhoneErrorMessage = "Phone number must be at least 10 numbers";
		}

		if( !$formValidations->isValidEmail($customerEmail) ) {
			$validForm = false;
			$customerEmailErrorMessage = "Invalid entry, please enter your email";
		}

		if( !$formValidations->validSelection($registrationType, array("attendee", "presenter", "volunteer", "guest")) ) {
			$validForm = false;
			$registrationTypeErrorMessage = "Please select a Registration Type";
		}

		if( !$formValidations->validSelection($badgeHolderType, array("badgeClip", "badgeLanyard", "badgeMagnet")) ) {
			$validForm = false;
			$badgeHolderTypeErrorMessage = "Please select a Badge Type";
		}

		if( !$formValidations->strLengthCheck(0, 200, $specialRequests) ) {
			$validForm = false;
...
?>
	...
	<div id="orderArea">
	<form name="orderAreaForm" method="post" action="customerRegistrationForm.php">
